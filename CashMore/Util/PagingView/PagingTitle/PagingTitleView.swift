...
@objc protocol PagingTitleViewDelegate {
    @objc optional func pagingTitleView(titleView: PagingTitleView, index: Int)
}

class PagingTitleView: UIView {
    init(frame: CGRect, titles:[String], configure: PagingTitleViewConfigure) {
    ...
    @objc weak var delegate: PagingTitleViewDelegate?

    /// 选中标题的下标，默认为 0
    @objc var index: Int = 0
    
    /// 重置选中标题的下标
    @objc func reset(index: Int) {
    ...
    private var configure: PagingTitleViewConfigure!
...
// MARK: initialization
private extension PagingTitleView {
    func initialization() {
